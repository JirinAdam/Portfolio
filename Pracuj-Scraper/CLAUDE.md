...
2. detail_scraper2.0.py              →  job_database.db           (DROP + fresh scrape + auto-snapshot CSV)
...
| `detail_scraper2.0.py` | Paralelní scraper — ThreadPoolExecutor, `--workers N` (default 32), fresh mode (DROP + rebuild), auto-snapshot |
...
| `snapshot_history.py` | Měsíční snapshot počtů do CSV (voláno automaticky z detail_scraper2.0.py) |
