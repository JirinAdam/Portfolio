...
5. snapshot_history.py               →  history CSVs              (denní snapshot počtů do CSV)
...
| `detail_scraper.py` | Scrape detailů každé nabídky → SQLite DB (sekvenční) |
| `detail_scraper2.0.py` | Paralelní verze — ThreadPoolExecutor, `--workers N` (default 8) |
...
| `snapshot_history.py` | Denní snapshot počtů do CSV (krok 5 pipeline) |
...
1. ~~**Paralelizace detail_scraper.py**~~ — **DOKONČENO** (`detail_scraper2.0.py`, ThreadPoolExecutor, `--workers N`, default 8)
