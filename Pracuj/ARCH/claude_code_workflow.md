...
## Status tasků (2026-03-17)

| Task | Popis | Status |
|------|-------|--------|
| TASK 0 | Inicializace projektu | DONE |
| TASK 1 | `data/loader.py` | DONE |
| TASK 2 | `components/charts.py` | DONE |
| TASK 3 | `app.py` — Skills Overview | DONE |
| TASK 4 | `pages/2_salary_per_role.py` | DONE |
| TASK 5 | `pages/3_salary_per_skill.py` | DONE |
| TASK 6 | `pages/4_salary_per_level.py` | DONE |
| TASK 7 | `pages/5_jobs_per_region.py` | DONE |
| TASK 8 | Deployment na Streamlit Cloud | DONE |
| TASK 9 | Visual customization (barvy, fonty, názvy) | PENDING |

### Deployment

- **GitHub:** `JirinAdam/Portfolio` → `Pracuj/dashboard/app.py`
- **Branch:** `main`

---

## TASK 9 — Visual Customization (pending)

**Rozsah:** Úprava vizuálního stylu nasazeného dashboardu.

### Customization points

| Co | Kde | Aktuální hodnota |
|----|-----|------------------|
| Barvy grafů (Plotly) | `components/charts.py` | `#636EFA` (modrá), `#EF553B` (oranžová) |
| Fonty grafů | `components/charts.py` | Plotly default |
| Globální téma Streamlit | `.streamlit/config.toml` `[theme]` | Žádné (default) |
| Názvy stránek v sidebar | Názvy souborů v `pages/` | `2_salary_per_role` atd. |
| Titul v záložce prohlížeče | `st.set_page_config(page_title=...)` | Různé per stránka |
| Hlavní nadpis stránky | `st.title(...)` | Anglické názvy |
| Custom CSS | Neexistuje | — |

### Postup (globální → specifické)

1. `.streamlit/config.toml` → `[theme]` sekce (primární barva, pozadí, font)
2. `components/charts.py` → Plotly barvy a fonty
3. Stránky → názvy, titulky, texty

---

## Rozhodnuté otázky

- Stránka 3 (Salary per skill): **absolutní počty** (count), filtr min 5 záznamů
- Vizuální téma: **TASK 9** — custom barvy/fonty (pending)
- Minimální počet záznamů pro salary: **5** (potvrzeno)
...
*Dokument verze 2.0 — TASK 0–8 dokončeny, TASK 9 pending*
