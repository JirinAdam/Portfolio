...
```
python main_trigger.py                  # full pipeline (or --skip-scrape, --workers N)

1. url_list_search2.0.py             →  job_urls_complete.json   (scrape URL z výsledků vyhledávání)
2. detail_scraper2.0.py              →  job_database.db           (DROP + fresh scrape, 32 workers)
3. db_cleaner/database_cleaner.py    →  job_database.db           (čištění a normalizace dat, interactive)
4. nerds_db_filter.py                →  nerd_jobs.db              (filtrování IT rolí)
5. snapshot_history.py               →  dashboard/data/*.csv      (měsíční snapshoty)
6. dashboard/TOP_Skills.py           →  Streamlit Cloud            (vizualizace nerd_jobs.db)
```
...
| `main_trigger.py` | Pipeline orchestrátor — spouští všech 5 kroků, `--skip-scrape`, `--workers N` |
| `detail_scraper2.0.py` | Paralelní scraper — ThreadPoolExecutor, `--workers N` (default 32), fresh mode (DROP + rebuild) |
...
| `snapshot_history.py` | Měsíční snapshot počtů do CSV (krok 5 v pipeline) |
...
| Soubor | Role |
|--------|------|
| `ARCH/` | Staré verze skriptů (`detail_scraper.py`, `url_scraper.py`, `nerds_database.py`), backup DB — celý adresář v .gitignore |
...
| `dashboard/components/charts.py` | 4 Plotly chart funkce (skills_bar, salary_bar, jobs_bar, trend_line) |
...
| `dashboard/pages/6_Trends.py` | Historical Trends (line chart z CSV history) |
...
- `job_database.db` je ~230 MB — velký soubor (gitignored), backup přesunut do ARCH/
...
1. ~~**Paralelizace detail_scraper.py**~~ — **DOKONČENO** (`detail_scraper2.0.py`, ThreadPoolExecutor, `--workers N`, default 32, fresh mode, auto-snapshot)
2. ~~**Historická data + trendové grafy**~~ — **DOKONČENO** (`snapshot_history.py`, `6_Trends.py`, CSV append logy)
3. **Material Symbols ikony** — `keyboard_double_arrow` se zobrazuje jako text místo ikon (sidebar, tabulky). Globální `* {font-family}` override přepisuje ikonkový font. CSS fix `.material-symbols-rounded` nefunguje.
4. **Odstranit "All Industries" dataset** z Trends stránky — ponechat pouze IT Roles
